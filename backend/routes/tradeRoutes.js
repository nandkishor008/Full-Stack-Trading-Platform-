  ...
  try {
    ...
    if (mode === "BUY") {
      if (position) {
        ...
        await PositionsModel.create({ userId, product: "CNC", name, qty, avg: price, price });
      }
    } else if (mode === "SELL") {
      if (position) {
        const newQty = position.qty - qty;
        position.avg = newQty !== 0 ? (position.avg * position.qty - price * qty) / newQty : 0;
        position.qty = newQty;
        await position.save();
      } else {
        await PositionsModel.create({ userId, product: "CNC", name, qty: -qty, avg: price, price });
      }
    }

    return res.status(200).json({ success: true, message: `${mode} executed successfully` });
  } catch (error) {
    console.error("Trade error:", error);
    return res.status(500).json({ success: false, error: "An error occurred during the trade." });
  }
});
...
router.get("/allHoldings", auth, async (req, res) => {
    try {
        const holdings = await HoldingsModel.find({ userId: req.userId }).sort({ _id: -1 });
...
router.get("/allPositions", auth, async (req, res) => {
    try {
        const positions = await PositionsModel.find({ userId: req.userId }).sort({ _id: -1 });
  ...
  try {
    ...
    for (const position of currentPositions) {
      ...
      if (holding) {
        ...
        if (totalQty !== 0) {
            holding.avg = ((holding.avg * holding.qty) + (position.avg * position.qty)) / totalQty;
        } else {
    ...
    res.status(200).json({ success: true, message: "Positions for the day have been settled to holdings." });

  } catch (error) {
    console.error("Settlement error:", error);
    res.status(500).json({ success: false, error: "An error occurred during settlement." });
  }
});
...
router.delete("/holdings/:id", auth, async (req, res) => {
  try {
    ...
    const holdingToDelete = await HoldingsModel.findOneAndDelete({ _id: id, userId: userId });

    if (!holdingToDelete) {
      return res.status(404).json({ error: "Holding not found or you do not have permission to remove it." });
    }

    res.status(200).json({ success: true, message: "Holding removed successfully." });

  } catch (error) {
    console.error("Error removing holding:", error);
    res.status(500).json({ success: false, error: "An error occurred while removing the holding." });
  }
});
